...
export default function ProgramFilterSidebar({ filters, onChange, onReset }: ProgramFilterSidebarProps) {
    const { t } = useLanguage();
    ...
    return (
        <aside className="w-full xl:w-80 shrink-0 bg-white rounded-xl border border-gray-100 shadow-sm p-5 flex flex-col gap-6 self-start">
            <p className="font-bold text-gray-900 text-base">{t("programs.filter.title")}</p>

            <div>
                <p className="text-sm font-semibold text-gray-700 mb-2">{t("programs.filter.search")}</p>
            ...
            <div>
                <p className="text-sm font-semibold text-gray-700 mb-3">{t("programs.filter.direction")}</p>
                <div className="flex flex-col gap-2">
                    {DIRECTIONS.map((dir) => (
                        <label key={dir} className="flex items-center gap-2.5 cursor-pointer group">
                            <input type="checkbox" checked={filters.directions.includes(dir)} onChange={() => toggleItem("directions", dir)} className="w-4 h-4 rounded border-gray-300 accent-blue-600 cursor-pointer" />
                            <span className="text-sm text-gray-600 group-hover:text-gray-900 transition-colors">{dir}</span>
                        </label>
                    ))}
                </div>
            </div>
            ...
            <div>
                <p className="text-sm font-semibold text-gray-700 mb-3">{t("programs.filter.level")}</p>
                <div className="flex flex-col gap-2">
                    {LEVELS.map((level) => (
                        <label key={level} className="flex items-center gap-2.5 cursor-pointer group">
                            <input type="checkbox" checked={filters.levels.includes(level)} onChange={() => toggleItem("levels", level)} className="w-4 h-4 rounded border-gray-300 accent-blue-600 cursor-pointer" />
