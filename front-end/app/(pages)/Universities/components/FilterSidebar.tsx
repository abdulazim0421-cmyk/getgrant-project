...
const COUNTRIES = ["США", "Великобритания", "Германия", "Франция", "Япония", "Австралия"];
...
export default function FilterSidebar({ filters, onChange, onReset }: FilterSidebarProps) {
    ...
    return (
        <aside className="w-full bg-white rounded-xl border border-gray-100 shadow-sm p-5 flex flex-col gap-6 self-start">
            ...
            <div>
                <p className="text-sm font-semibold text-gray-700 mb-3">Тип университета</p>
                <div className="flex flex-col gap-2">
                    {TYPES.map((type) => (
            ...
            <div>
                <p className="text-sm font-semibold text-gray-700 mb-3">Стоимость обучения (в год)</p>
                <input
                    type="range"
                    min={0}
                    max={100000}
