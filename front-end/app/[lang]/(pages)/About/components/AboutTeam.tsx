...
function TeamCard({ member, delay }: { member: TeamMember; delay: number }) {
    return (
        <motion.div className="flex items-start gap-4 p-5 bg-white border border-[#EAECF0] rounded-2xl shadow-sm transition-all duration-300 hover:shadow-md hover:border-blue-100" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay }}>
            <div className="w-[80px] h-[80px] flex-shrink-0 rounded-2xl overflow-hidden bg-gray-200">
                <img src={member.avatar} alt={member.name} className="w-full h-full object-cover transition-transform duration-300 hover:scale-105" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
            </div>
    ...
    return (
        <section className="py-20">
            <div className="max-w-7xl mx-auto px-6 lg:px-12">
                <h2 className="text-3xl font-bold text-slate-900">{t("about.team.title")}</h2>
                <p className="mt-2 text-sm text-slate-500 mb-10">{t("about.team.subtitle")}</p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    {team.map((m, i) => (<TeamCard key={m.name} member={m} delay={i * 0.08} />))}
                </div>
            </div>
        </section>
    );
